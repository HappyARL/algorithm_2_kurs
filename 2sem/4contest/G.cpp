...
class Graph {
 private:
  std::vector<int64_t> adj_list_[kMax];
 ...
 public:
  ...
  Graph(int64_t t);
  ~Graph();
  void  AddEdge(int64_t src, int64_t to);
  bool BFS();
...
void Graph:: AddEdge(int64_t src, int64_t to) {
  int64_t m = edges.size();
  adj_list_[src].push_back(m - 2);
  adj_list_[to].push_back(m - 1);
}

bool Graph::BFS() {
  visited_.assign(kMax, false);
  std::queue<int64_t> queue;
  queue.push(1);
  level_[1] = 0;
  visited_[1] = true;
  while (!queue.empty()) {
    int64_t vert_u = queue.front();
    queue.pop();
...
int main() {
  ...
  for (int64_t i = 0; i < edge_num; i++) {
    ...
    graph.edges.push_back(one);
    Edge two{vert_a, vert_b, 0, 0};
    graph.edges.push_back(two);
    graph.AddEdge(vert_a, vert_b);
  }
